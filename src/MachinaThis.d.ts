...
type EventHandlerFn<F extends Fsm, EventName extends '*' | keyof InternallyEmittedEvents<F> | keyof F["emittableEvents"]> =
    EventName extends keyof InternallyEmittedEvents<F>
        ? (this: MachinaThisInitializeFn<F> | MachinaThis<F>, ...args: GetEventArguments<InternallyEmittedEvents<F>[EventName]>) => any
        : EventName extends keyof F["emittableEvents"]
            ? (this: MachinaThisInitializeFn<F> | MachinaThis<F>, ...args: GetEventArguments<F["emittableEvents"][EventName]>) => any
            : (this: MachinaThisInitializeFn<F> | MachinaThis<F>, eventName: keyof InternallyEmittedEvents<F> | keyof F["emittableEvents"], ...data: GetEventArguments<InternallyEmittedEvents<F>[keyof InternallyEmittedEvents<F>]> | GetEventArguments<F["emittableEvents"][keyof F["emittableEvents"]]>) => any;
...
export type MachinaThisInitializeFn<F extends Fsm> = {
    ...
    handle<EventName extends keyof F["events"]>(eventName: EventName, ...args: GetEventArguments<F["events"][EventName]>): void,
    emit<EventName extends keyof F["emittableEvents"]>(eventName: EventName, ...args: GetEventArguments<F["emittableEvents"][EventName]>): void,
    on<EventName extends '*' | keyof InternallyEmittedEvents<F> | keyof F["emittableEvents"]>(eventName: EventName, fn: EventHandlerFn<F, EventName>): void,
    off<EventName extends '*' | keyof InternallyEmittedEvents<F> | keyof F["emittableEvents"]>(eventName: EventName, fn: EventHandlerFn<F, EventName>): void,
...
export type CurrentAction<F extends Fsm> = '' | `${keyof F["states"] extends string | number ? keyof F["states"] : never}.${keyof F["events"] extends string | number ? keyof F["events"] : never}`;
